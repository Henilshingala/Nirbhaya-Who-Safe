...
class ContactRepository(private val contactDao: ContactDao) {
    ...
    suspend fun globalDeleteContact(contact: EmergencyContact) {
        withContext(Dispatchers.IO) {
            try {
                contactDao.globalDeleteContact(contact)
            } catch (e: Exception) {
                android.util.Log.e("ContactRepository", "Error deleting global contact", e)
            }
        }
    }
     ...
     * Used when clearing all app data
     */
    suspend fun deleteAllContacts() {
        withContext(Dispatchers.IO) {
            try {
                contactDao.deleteAllContacts()
                android.util.Log.d("ContactRepository", "All contacts deleted")
            } catch (e: Exception) {
                android.util.Log.e("ContactRepository", "Error deleting all contacts", e)
            }
        }
    }
}
